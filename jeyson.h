...
void jeyson_log_error(Jeyson_Parser* parser)
{
	...
	for (int k = line_index; k < 0; k++)
	{
		String next_line;
		if (jeyson_get_line_relative_index(&next_line, &parser->source, parser->pos, k))
		{
...
void jeyson_pack_string(String* string, WriteBuffer* string_buffer)
{
	String s;
	s.buf = (char*)string_buffer->base;
	s.len = (U32)string_buffer->pos;
...
void jeyson_pack_strings(Jeyson_Element* element, WriteBuffer* string_buffer)
{
	while (element)
	{
		jeyson_pack_string(&element->label, string_buffer);
		if (element->type == JEYSON_TYPE_STRING)
		{
			jeyson_pack_string(&element->value.string, string_buffer);
		}
		else if (element->type == JEYSON_TYPE_ELEMENT || element->type == JEYSON_TYPE_ARRAY)
		{
			jeyson_pack_strings(element->value.child, string_buffer);
		}
		element = element->next;
	}
}
...
Jeyson_Element* jeyson_parse_source(Arena* arena, char const* data, U32 size)
{
	...
	if (parser.source.buf[0] == '{' && jeyson_parse_value(&parser, root))
	{
		...
		jeyson_pack_strings(root, &string_buffer);
